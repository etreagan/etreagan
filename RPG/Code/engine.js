...
function lootlist() {
	...
	gainmoney = (Math.floor(Math.random() * enemy.lootmoney[1]) + enemy.lootmoney[0]);
		if (gainmoney > 0) {
			money += gainmoney;
			if (gainmoney > 1) {
				addline("<invmun><img class='invmunimg' src='Graphics/" + moneygfx + "' ><center><invmunn>And "+ gainmoney + " " + pcurrencydef +"!</invmunn></center></invmun>");
			} else {
				addline("<invmun><img class='invmunimg' src='Graphics/" + moneygfx + "' ><center><invmunn>And "+ gainmoney + " " + currencydef +"!</invmunn></center></invmun>");
			};
		};
};
	};
